...
float bias3[5];
float alpha=0.1;
float weights1_v[3][4];
float weights2_v[4][5];
float weights3_v[5];
float bias1_v[4][4];
float bias2_v[4][5];
float bias3_v[5];
...
void main()
{
    ...
    for(epoch=0;epoch<5000;epoch++)
    {
        ...
        for (i=0;i<3;i++)
        {
            for(j=0;j<4;j++)
            {
                weights1_v[i][j]=gammaq*weights1_v[i][j] + alpha*d_weights1[i][j];
        ...
        for (i=0;i<4;i++)
        {
            for(j=0;j<5;j++)
            {
                weights2_v[i][j]=gammaq*weights2_v[i][j] + alpha*d_weights2[i][j];
                weights2[i][j] +=weights2_v[i][j];
            }
        }
        for(j=0;j<4;j++)
        {
                weights3_v[j]=gammaq*weights3_v[j] + alpha*d_weights3[j];
        ...
        for (i=0;i<4;i++)
        {
            for(j=0;j<4;j++)
            {
                 bias1_v[i][j]=gammaq*bias1_v[i][j] + alpha*interm3[i][j];
        ...
        for (i=0;i<4;i++)
        {
            for(j=0;j<5;j++)
            {
                 bias2_v[i][j]=gammaq*bias2_v[i][j] + alpha*interm2[i][j];
        ...
        for(j=0;j<4;j++)
        {
                 bias3_v[j]=gammaq*bias3_v[j] + alpha*interm1[j];
                 bias3[j] +=bias3_v[j];
        }
        printf("\n");
    }
